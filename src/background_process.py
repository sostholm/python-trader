...
async def update_coin(collection, updates):
    return await collection.update_many('id':)
...
async def coin_gecko():
    ...
    # await client.trader.coin_gecko.update_one({'_id': coin_gecko['_id']}, {'$set': {'loop_state':'running'}} , upsert=False)
        ...
        while coin_gecko['loop_state'] == 'running':
            ...
            if subscriptions != '':
                ...
                if response.ok:
                    ...
                    await update_gecko(gecko_collection, coin_gecko, {'coin_list': coin_list})
...
async def background_user_sync(app, user):
        ...
        while user['loop_state'] == "running":
            ...
            if hasattr(user, 'accounts'):
                ...
                if result.data:
                    ...
                    update_user(user_collection, user, {'last_update': datetime.now()})
    ...
    await update_user(user_collection, user, {'loop_state': "stopped"}, '$push')
